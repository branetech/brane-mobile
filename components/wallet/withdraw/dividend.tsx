...
import {SuccessPage} from '@/components/Screens';
import Dividend from '@/components/wallet/dividen';
import DividendAcc from '@/components/wallet/dividen/DividenAcc';
...
const DiviWallet = () => {
    ...
    const handleSubmit = () => setStage('acc')
    const handleOtpSubmit = () => setStage('success')


    return (
            {stage === '' && <Dividend onSubmit={handleSubmit}/>}
            {stage === 'acc' && <DividendAcc onSubmit={handleOtpSubmit}/>}
            {stage === 'success' && <SuccessPage title='Your withdrawal was successful.' bg='#013D25' color='#D2F1E4' text='You’ve successfully withdrawn <span class="text-[#342A3B]">₦3,000</span> to your bank account. ' btnText="Dismiss" onBtnClick={() => router.push('/home')}/>}
        </>
    )
}

export default DiviWallet
...
import { AUTH_SERVICE, STOCKS_SERVICE, TRANSACTION_SERVICE } from "@/services/routes";
import {
    hideAppLoader,
    priceFormatter,
    showAppLoader,
...
export default function DividendWithdrawScreen() {
    ...
    const handleContinueForm = () => {
        ...
        if (!amount || num <= 0) { setAmountError("Enter a valid amount"); return; }
        if (num > dividendBalance) { setAmountError("Insufficient dividend balance"); return; }
    ...
    const onPinValidated = async () => {
        ...
        try {
            const res: any = await BaseRequest.post(
                ...
                { amount: Number(amount), accountNumber: selectedAccount?.accountNumber },
            );
                ...
                >
                    ...
                    >
                        ...
                        <FormInput
                            ...
                            value={amount}
                            onChangeText={(t) => { setAmount(t); setAmountError(undefined); }}
                            error={amountError}
        ...
        return (
            <SafeAreaView style={[s.screen, { backgroundColor: C.background }]}>
                ...
                {isLoadingAccounts ? (
                    ...
                    <FlatList
                        data={accounts}
                        keyExtractor={(item, i) => String(item?.id ?? item?.accountNumber ?? i)}
                        contentContainerStyle={s.content}
                        showsVerticalScrollIndicator={false}
                        ListEmptyComponent={
    ...
    return (
        ...
        >
            <View style={[s.successCard, { backgroundColor: C.inputBg }]}>
                ...
                <ThemedText type="subtitle" style={{ color: C.text, marginTop: 16, textAlign: "center" }}>
                    Withdrawal Successful
                </ThemedText>
                <ThemedText style={{ color: C.muted, textAlign: "center", marginTop: 8, fontSize: 14, lineHeight: 22 }}>
...
const s = StyleSheet.create({
    ...
    accountRow:     { flexDirection: "row", alignItems: "center", justifyContent: "space-between", borderWidth: 1, borderRadius: 14, paddingVertical: 14, paddingHorizontal: 16 },
    accountInfo:    { flex: 1 },
    successWrapper: { justifyContent: "center", alignItems: "center", paddingHorizontal: 24 },
    successCard:    { width: "100%", borderRadius: 20, padding: 28, alignItems: "center", gap: 0 },
});
'use client'
