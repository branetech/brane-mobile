...
const ManagedWealthPage: React.FC = () => {
  ...
  //console.log(investmentData, "managed bracs")

  const transformedInvestmentData = Array.isArray(investmentData) ? investmentData.map((data: any) => ({
    ...
    <div className="mx-auto px-5 pb-10 bg-white min-h-screen">
      ...
      <main className="">
        ...
        <section className="mb-8">
          ...
          {transformedInvestmentData && transformedInvestmentData.map((data: any, index: number) => {
              const visualIndex =
                index < activeIndex
